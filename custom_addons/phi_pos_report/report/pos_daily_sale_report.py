...
class ReportDailySaleDetails(models.AbstractModel):
    _name = 'report.phi_pos_report.report_pos_daily_saledetails'
    _description = 'Point of Sale Daily Details'

    @api.model
    def get_sale_details(self, date_report=False, config_ids=False):
        ...
        orders_prev_year = [order for order in orders if order.date_order.date() == (
                dt.datetime.strptime(date_report, "%Y-%m-%d") + relativedelta(years=-1)).date()]
    ...
    def _get_report_values(self, docids, data=None):
        ...
        data.update(self.get_sale_details(data['date_report'], configs.ids))
        return data
